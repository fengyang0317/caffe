...
def wide_basic(net, from_layer, prefix, nInputPlane, nOutputPlane, stride):
    ...
    net[branch2b_name] = L.Convolution(net[relu_branch2b_name], num_output=nOutputPlane, kernel_size=3, pad=1,
                                       stride=1, **c1_kwargs)
    if nInputPlane != nOutputPlane or stride != 1:
...
def WideResNetBody(net, from_layer, depth = 10, widen_factor = 1):
    ...
    n = (depth - 4) / 6
    nStages = [16, 16 * widen_factor, 32 * widen_factor, 64 * widen_factor, 128 * widen_factor,
                256 * widen_factor, 256 * widen_factor, 256 * widen_factor]
    ...
    layer(net, 'res7a_{}'.format(n-1), 'res8a', nStages[6], nStages[7], n, 2)
    net['bn'] = L.BatchNorm(net['res8a_{}'.format(n-1)], in_place=True, **bn_kwargs)
